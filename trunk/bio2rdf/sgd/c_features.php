...
define("PS","<");
define("PE",">");
define("ONTO","http://bioonto.de/elvira.owl#");
define("PART_OF", PS.ONTO."part-of".PE);
define("HAS_PHENOTYPE",PS.ONTO."has-phenotype".PE);
define("HAS_FUNCTION", PS.ONTO."has-function".PE);
define("PARTICIPATES_IN", PS.ONTO."participates-in".PE);
define("IS_ENCODED_BY",PS.ONTO."is-encoded-by".PE);
...
class SGD_C_FEATURES {
	...
	function Convert2RDF()
	{
		...
		while($l = fgets($this->_in,2048)) {
			...
			if(trim($a[15])) $buf .= "$sgd:$id $dc:description ".'"""'.trim($a[15]).'""" .'.PHP_EOL;
			$feature_type = $this->GetFeatureType($a[1]);
			$buf .= "$sgd:$id rdfs:subClassOf <http://bio2rdf.org/".strtolower($feature_type).">. ".PHP_EOL;
			...
			if(isset($type)) {
				unset($p1);unset($p2);
				$gp = $sgd.":".$id."gp";
				$buf .= "<http://bio2rdf.org/$sgd:$gp> rdfs:subClassOf [a owl:Restriction;owl:onProperty ".IS_ENCODED_BY.";owl:someValuesFrom <http://bio2rdf.org/$sgd:$id>.].".PHP_EOL;
				$buf .= "<http://bio2rdf.org/$gp> rdfs:label \"$id"."gp [$gp]\".".PHP_EOL;
				if($type == "p") $buf .= "<http://bio2rdf.org/$gp> rdfs:subClassOf <http://bio2rdf.org/chebi:36080> .".PHP_EOL;
				elseif($type == "r") $buf .= "<http://bio2rdf.org/$gp> rdfs:subClassOf <http://bio2rdf.org/chebi:33697> .".PHP_EOL;

				if($a[1] == "ORF" && $a[3] != '') {
					$p1 = ucfirst(strtolower(str_replace(array("(",")"), array("&#40;","&#41;"), $a[3])))."p";
					$buf .= "<http://bio2rdf.org/$sgd:$p1> rdfs:subClassOf [a owl:Restriction;owl:onProperty ".IS_ENCODED_BY.";owl:someValuesFrom <http://bio2rdf.org/$sgd:$id>.].".PHP_EOL;
//					$buf .= "<http://bio2rdf.org/$sgd:$p1> owl:equivalentClass <http://bio2rdf.org/$gp>.".PHP_EOL;
					$buf .= "<http://bio2rdf.org/$sgd:$p1> rdfs:label \"$p1 [$sgd:$p1]\".".PHP_EOL;
					$buf .= "<http://bio2rdf.org/$sgd:$p1> rdfs:subClassOf <http://bio2rdf.org/chebi:36080> .".PHP_EOL;
				}
				if($a[1] == "ORF" && $a[4] != '') {
					$p2 = ucfirst(strtolower(str_replace(array("(",")"), array("&#40;","&#41;"), $a[4])))."p";
					$buf .= "<http://bio2rdf.org/$sgd:$p2> rdfs:subClassOf [a owl:Restriction;owl:onProperty ".IS_ENCODED_BY.";owl:someValuesFrom <http://bio2rdf.org/$sgd:$id>.].".PHP_EOL;
//					$buf .= "<http://bio2rdf.org/$sgd:$p2> owl:equivalentClass <http://bio2rdf.org/$gp>.".PHP_EOL;
					$buf .= "<http://bio2rdf.org/$sgd:$p2> rdfs:label \"$p2 [$sgd:$p2]\".".PHP_EOL;
					$buf .= "<http://bio2rdf.org/$sgd:$p2> rdfs:subClassOf <http://bio2rdf.org/chebi:36080> .".PHP_EOL;
				}
				if(isset($p1) && isset($p2)) 
					$buf .= "<http://bio2rdf.org/$sgd:$p1> owl:equivalentClass <http://bio2rdf.org/$sgd:$p2>.".PHP_EOL;
			}
...
//				$buf .= "$sgd:$id owl:equivalentClass <http://bio2rdf.org/$sgd:$nid> .".PHP_EOL;
			}
...
//				$parent = urlencode($a[6]);
				...
				$buf .= "$sgd:$id rdfs:subClassOf [a owl:Restriction; owl:onProperty ".PART_OF.";owl:someValuesFrom <http://bio2rdf.org/$sgd:$parent>.] .".PHP_EOL;
				if(strstr($parent,"chromosome")) {
					$parent_type = 'c';
					if(!isset($chromosomes[$parent])) $chromosomes[$parent] = '';
					else {
						$other .= "$sgd:$parent rdfs:subClassOf <http:bio2rdf.org/so:0000340> .".PHP_EOL;
...
//						$buf .= "$sgd:$id $owl:sameAs $sgd:$c.".PHP_EOL;
			...
			if($a[8] && $parent_type != 'c') {
				$chr = "chromosome_".$a[8];
				$buf .= "$sgd:$id rdfs:subClassOf [a owl:Restriction; owl:onProperty ".PART_OF."; owl:someValuesFrom <http://bio2rdf.org/$sgd:$chr>.] .".PHP_EOL;
			}
			...
			if($a[11]) {
				...
				$buf .= "$sgd:$id rdfs:subClassOf [ a owl:Restriction;owl:onProperty ".PART_OF.";owl:someValuesFrom $sgd:$strand .].".PHP_EOL;
				if(!isset($strands[$strand])) {
					$strands[$strand] = '';
					$other .= "$sgd:$strand a sgd:$strand_type .".PHP_EOL;
					$other .= "$sgd:$strand rdfs:label \"$strand_type for $chr\" .".PHP_EOL;
					$other .= "$sgd:$strand rdfs:subClassOf [a owl:Restriction; owl:onProperty ".PART_OF."; owl:someValuesFrom <http://bio2rdf.org/$sgd:$chr>.] .".PHP_EOL;
				}
			}
