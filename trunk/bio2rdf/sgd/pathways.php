	...
	function Convert2RDF()
	{
		...
		while($l = fgets($this->_in,96000)) {
			...
			if(!isset($e[$pid])) {
				...
				if(!$sp) $buf .= "$sgd:$pid a $sgd:Pathway .".PHP_EOL;
				else $buf .= "$sgd:$pid a $sgd:Superpathway .".PHP_EOL;
			}
			if($sp) { // add the pathway to the superpathway
				$pathway = substr($a[1],0,-(strlen($a[1])-strrpos($a[1]," ")));
				$buf .= "$sgd:$pid $ss:SIO_000053 $sgd:".md5($pathway).".".PHP_EOL;
				continue;
			}
			...
			if($a[3]) { // there is a protein
				...
				$buf .= "$sgd:$pid $ss:SIO_000132 <http://bio2rdf.org/$sgd:$eid>.".PHP_EOL; 
			}				
			$cid = '';
			if($a[1]) { // enzyme complex
				...
				$buf .= "$sgd:$pid $ss:SIO_000132 <http://bio2rdf.org/$sgd:$cid>.".PHP_EOL;
				if($eid) $buf .= "$sgd:$cid $ss:SIO_000053 <http://bio2rdf.org/$sgd:$eid> .".PHP_EOL;	
			}
			if($a[2]) { // EC reaction
				$buf .= "$sgd:$pid $ss:SIO_000053 $ec:$a[2] .".PHP_EOL;
				$buf .= "$ec:$a[2] $rdfs:label \"$a[2] [$ec:$a[2]]\" .".PHP_EOL;
				$buf .= "$ec:$a[2] a $sgd:Reaction .".PHP_EOL;
				$buf .= "$ec:$a[2] $ss:SIO_000132 <http://bio2rdf.org/$sgd:$eid>.".PHP_EOL;
				if($cid) $buf .= "$ec:$a[2] $ss:SIO_000132 <http://bio2rdf.org/$sgd:$cid> .".PHP_EOL;				
			}
			...
			if($a[4]) { // publications
				...
				foreach($b AS $c) {
					...
					$buf .= "$sgd:$pid $ss:SIO_000212 $ns:$d[1].".PHP_EOL;
				}
			}
