...
# --- UCSF Chimera Copyright ---
#
# $Id: v38.py 26655 2009-01-07 22:02:30Z gregc $

from v38 import RemapDialog, reportRestoreError, restoreWindowSize, \
	...
	restoreMolecules, expandSummary, expandSequentialSummary, \
        restoreSurfaces, checkVersion, restoreViewer
...
def restoreModelAssociations(associations):
	for mid, models in associations.items():
		owner = idLookup(mid)
		for a in models:
			owner.addAssociatedModel(idLookup(a))
...
def setSessionIDparams(molecules, residues, atoms, bonds, surfaces, vrmls,
	...
	for i, item in enumerate(molecules+residues+atoms+bonds
					+surfaces+vrmls+pseudobonds+pbGroups):
		sessionMap[item] = i
