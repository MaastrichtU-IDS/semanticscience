...
meshDir = ''
pocketMap = {}
def makeAllMeshes(proID, peptideLen=40, pocketDist=5.0):
   ...
   # os.system("chmod 777 " + meshDir)  
    try:
        ...
        print '\nRunning Delphi................'
        runCommand('~select')
        runCommand('write format pdb 0 '+ proID + 'H.pdb')  #Write delphi-ready protein with hydrogens
        
        #Create delphi parameter file
        f = open('./param_1', 'w')
        f.write('in(pdb,file="'+ proID + 'H.pdb")\n')       #Input/Output files
        f.write('in(siz,file="amber.siz")\n')
        f.write('in(crg,file="amber.crg")\n')
        f.write('out(phi,file="'+ proID + '.phi")\n')
      ...
      #  f.write('nonit=20\n')                               #Non-linear iterations
        f.close()
        os.system("./runDelphi")                            #Run delphi
