...
try:
	...
	from numpy import zeros as num_zeros
	from numpy import concatenate as num_concatenate
	from numpy import logical_not as num_logical_not
	from numpy import choose as num_choose
	from numpy import sum as num_sum
	from numpy import nonzero as num_nonzero
	from numpy import take as num_take
	from numpy import reshape as num_reshape
	from numpy import greater_equal as num_greater_equal
	from numpy import equal as num_equal
	from numpy import float32 as num_float32
	from numpy import int32 as num_int32
except ImportError:
	import Numeric
	from Numeric import array as num_array
	from Numierc import zeros as num_zeros
	from Numeric import concatenate as num_concatenate
	from Numeric import logical_not as num_logical_not
	from Numeric import choose as num_choose
	from Numeric import sum as num_sum
	from Numeric import nonzero as num_nonzero
	from Numeric import take as num_take
	from Numeric import reshape as num_reshape
	from Numeric import greater_equal as num_greater_equal
	from Numeric import equal as num_equal
	num_float32 = 'f'
	num_int32 = 'i'
import msmsc
import msms
...
def readxyzr(filename):
    ...
    not provide names"""
    return msmsc.MS_readxyzr(filename)
...
class MSMS(msms.MOLSRF):
    ...
    def compute(self, probe_radius=1.5, density=1.0):
    ...
    the latter case an error message is available in MS_ERR_MSG"""
        return msmsc.MS_compute_surface(self.this, probe_radius, density)
    ...
    """
        ...
        if probe_radius != self.rp:
            ...
            if self.rsr.nb !=0: # new probe radius ==> delete RS, SES
                ...
                msmsc.MS_reset_RSR(self.this)
                s = self.sesr.fst
                while s.this:
                    msmsc.MS_free_triangulated_surface(s.this)
                    s=s.nxt
                msmsc.MS_reset_SESR(self.this)

                rsr = self.rsr  # initialize RSR
                rsr.fst = msms.RSR(None)
                rsr.lst = msms.RSR(None)
                rsr.nb = 0
                rsr.nb_free_vert = 0
                rsr.ffnba = 0
                rsr.set_ffat( -1, -1, -1 )
                ...
                sesr.fst = msms.SESR(None)
                sesr.lst = msms.SESR(None)
                sesr.nb = 0
        ...
        if allComponents != self.all_components:
            msg = "Only 0 and 1 are valide for allComponents"
    ...
    """
        ...
        if component==None:
            rs = self.rsr.fst
            while rs.this:
                msmsc.MS_solvent_excluded_surface(self.this, rs.this)
                rs=rs.nxt
        else:
            rs = self._getRSComp(component).this
            msmsc.MS_solvent_excluded_surface(self.this, rs)


    def triangulate_one_component(self, rs, density=1.0):
    ...
    density
        ...
        if density != ses.density:
            msmsc.MS_free_triangulated_surface(ses.this)
            msmsc.MS_set_SES_density( ses.this, density, self.rp )

        if ses.nbtri:
            raise RuntimeError, "This SES component is already triangulated"

        status = msmsc.MS_make_templates( self.this, ses.this )
        if status != msms.MS_OK:
            raise RuntimeError, "Could not construct triangulation templates"

        status = msmsc.MS_triangulate_SES_component( self.this, rs.this )
        if status != msms.MS_OK:
    ...
    """
        f = msmsc.MS_write_triangulated_SES_component
    ...
    def getTriangles(self, atomIndices=None, selnum=1, base=0, component=0):
    ...
    component
        ...
        comp = self._getSESComp(component).this
        if atomIndices:
            return msmsc.MS_get_triangles(self.this, comp, atomIndices,
                                          selnum=selnum, base=base)
        else:
            return msmsc.MS_get_triangles(self.this, comp,
                                          selnum=selnum, base=base)
    ...
    """
        ...
        return msmsc.MS_compute_SES_area(self.this)


    def compute_numeric_area_vol(self, component=None,
                                 mode=msmsc.MS_SEMI_ANALYTICAL):
        """compute_numeric_area_vol(component=None,mode=msmsc.MS_SEMI_ANALYTICAL)
    ...
    """
        ...
        assert mode in (msmsc.MS_NUMERICAL, msmsc.MS_SEMI_ANALYTICAL,
                        msmsc.MS_BOTH,)
        ...
        if component is None:
            ...
            while rs.this:
                if rs.ses.this and rs.ses.nbtri>0:
                    msmsc.MS_compute_numerical_area_vol( self.this,
                                                         rs.ses.this, mode)
                    i = i + 1
                rs=rs.nxt
        else:
            comp = self._getSESComp(component).this
            msmsc.MS_compute_numerical_area_vol(self.this, comp, mode)
    ...
    """
        if component is None:
            ...
            while rs.this:
                if rs.ses.this and rs.ses.nbtri>0:
                    _filename = "%s_%d"%(filename, i)
                    msmsc.MS_write_surface_areas(self.this, _filename, i)
                    i = i + 1
                rs=rs.nxt
        else:
            msmsc.MS_write_surface_areas(self.this, filename, component)
    ...
    """
        return msmsc.MS_detailed_info_molsrf(self.this)
    ...
    """
        ...
        return msmsc.MS_info_molsrf(self.this)
    ...
    """
        ...
        if component is None:
            ...
            while rs.this:
                if rs.ses.this and rs.ses.nbtri>0:
                    msmsc.MS_resetBuriedVertexFlags(rs.ses.this)
                    i = i + 1
                rs=rs.nxt
        else:
            comp = self._getSESComp(component).this            
            msmsc.MS_resetBuriedVertexFlags(comp)
...
"""
    ...
    """
        if component is None:
            ...
            while rs.this:
                if rs.ses.this and rs.ses.nbtri>0:
                    msmsc.MS_resetBuriedVertexArea(rs.ses.this)
                    i = i + 1
                rs=rs.nxt
        else:
            comp = self._getSESComp(component).this            
            msmsc.MS_resetBuriedVertexArea(comp)


    def buriedSurfaceArea(self, component=None, mode=msmsc.MS_SEMI_ANALYTICAL):
        """buriedSurfaceArea(component=None,mode=msmsc.MS_SEMI_ANALYTICAL)
    ...
    """
        ...
        if component is None:
            ...
            while rs.this:
                if rs.ses.this and rs.ses.nbtri>0:
                    msmsc.MS_vertexBuriedSurfaceArea( rs.ses.this )
            ...
            msmsc.MS_vertexBuriedSurfaceArea(comp.this)
    ...
    """
        ...
        buriedFlag = vint[:,2]
        if negate:
            buriedFlag = num_logical_not(buriedFlag)
        triBuried = num_choose(tri[:,:3], buriedFlag)
        sum = num_sum(triBuried, 1)
        faceInd = num_nonzero( num_greater_equal(sum, selnum) )
        faces = num_take(tri, faceInd)
    ...
    """
        ...
        msmsc.MS_reset_atom_update_flag( self.this )
        msmsc.MS_tagCloseProbes(self.this, self.rsr.fst, cut)
    ...
    """
        msmsc.MS_reset_atom_update_flag( self.this )
        msmsc.MS_get_xyzr_update(self.this, filename, max)
    ...
    indiced: indices of moving atoms
        """
        msmsc.MS_reset_atom_update_flag( self.this )
        msmsc.MS_updateSpheres(self.this, len(coords), indices, coords)
    ...
    """
        ...
        stat = msmsc.MS_update_surface(self.this, self.rsr.fst, mode,
                                       density, update)
...
#	    print "ERROR while updating RS %d %s\n" % (update ,msms.MS_err_msg)
    ...
    def updateSurfaceArea(self):
        ...
        re-computed.
        """
        return msmsc.MS_update_SES_area(self.this, self.rsr.fst.ses)
    ...
    def update(self, mode=msms.FULL, density=None, nup=0):
        ...
        assert mode in (msms.FULL, msms.TORIC, msms.ALL_WITH_DENSITY,
                        msms.NO_TRIANGULATION)
        rs = self.rsr.fst
        rs.fstrfup = msms.RSF(None)
        rs.lstrfup = msms.RSF(None)
        rs.fstreup = msms.RSE(None)
        rs.lstreup = msms.RSE(None)
        rs.fstRSbfup = msms.RSE(None)
        rs.lstRSbfup = msms.RSE(None)
        rs.fstfup = msms.SESF(None)
        rs.lstfup = msms.SESF(None)
        rs.fsteup = msms.SESE(None)
        rs.lsteup = msms.SESE(None)

        su = rs.ses
        su.fsttfup = msms.SESF(None)
        su.lsttfup = msms.SESF(None)
        su.fsttorup = msms.SESF(None)
        su.lsttorup = msms.SESF(None)
        if self.try_num > 1:
            msmsc.MS_restore_radii(self.this, rs.this)

        i = msmsc.MS_update_reduced_surface(self.this,rs.this,nup)
        if i==msms.MS_ERR:
            raise RuntimeError, "ERROR while updating RS %s\n"%msms.MS_err_msg
  
        su.lsesv = msms.SESV(None)
        su.lsese = msms.SESE(None)
        su.lsesf = msms.SESF(None)
        i = msmsc.MS_update_ses(self.this,rs.this,nup)
        if i==msms.MS_ERR:
            raise RuntimeError, "ERROR while updating SES %s\n"%msms.MS_err_msg

        i = msmsc.MS_update_triangulation_SES_component(self.this, rs.this,
                                                        mode, density, nup)
        if i==msms.MS_ERR:
            raise RuntimeError, "ERROR while updating SES %s\n"%msms.MS_err_msg
        ...
        if component is None:
            ...
            while rs.this:
                if rs.ses.this and rs.ses.nbtri>0:
                    ...
                    col.append( num_take(colors, tri[:,3]-1) )
                    i = i + 1
                rs=rs.nxt
        else:
            comp = self._getSESComp(component).this            
            dum, dum, tri = self.getTriangles(component=component)
            col = num_take(colors, tri[:,3]-1)
        return col


##      def getAnaFaceTriangles(self, num):
##  	"""returns the set of triangles of an analytical SES face"""
##  	fact = num_equal( self.sesf[:,4]-1, num )
##  	find = num_nonzero(fact)
##  	if len(find):
##  	    fac = num_take( self.sesf, find )
##  	    vertsFlag = num_zeros( (len(self.sesv),), 'i' )
##  	    for f in fac:
##  		vertsFlag[f[0]] = 1;
##  		vertsFlag[f[1]] = 1;
##  		vertsFlag[f[2]] = 1;
##  	    vind = num_nonzero(vertsFlag)
##  	    vts = num_take( self.sesv, vind )
##  	return vts, fac
...
##  	    mat = self.sesfColors
##  	elif faces == 'toric':
##  	    fact = num_equal( self.sesf[:,3], 3 )
##  	    ind = num_nonzero(fact)
##  	    fac = num_take( self.sesf[:,:3], ind )
##  	    mat = num_take( self.sesfColors, ind )
##  	elif faces == 'reent':
##  	    fact = num_equal( self.sesf[:,3], 2 )
##  	    ind = num_nonzero(fact)
##  	    fac = num_take( self.sesf[:,:3], ind )
##  	    mat = num_take( self.sesfColors, ind )
##  	elif faces == 'contact':
##  	    fact = num_equal( self.sesf[:,3], 1 )
##  	    ind = num_nonzero(fact)
##  	    fac = num_take( self.sesf[:,:3], ind )
##  	    mat = num_take( self.sesfColors, ind )
##  	else:
##  	    assert len(faces) > 0
##  	    if analytical == 0 :
##  		fac = num_take( self.sesf[:,:3], list(faces) )
##  		mat = num_take( self.sesfColors, list(faces) )
##  	    else:
##  		fact = num_equal( self.sesf[:,4], faces[0] )
##  		for fi in faces[1:]:
##  		    fact = fact + num_equal( self.sesf[:,4], fi )
##  		ind = num_nonzero(fact)
##  		fac = num_take( self.sesf[:,:3], ind )
##  		mat = num_take( self.sesfColors, ind )
...
##  	triCenters = num_take( self.sesv[:,:3], self.sesf[:,:3] )
##  	triCenters = num_sum(triCenters, 1)/3.0
...
##  	else:
##  	    assert len(triang) > 0
##  	    labs = num_take( atlabs, list(triang) )
##  	    pos = num_take( triCenters, list(triang) )
...
##  	for fi in faces:
##  	    fact = num_equal( anaFacesNum, fi )
##  	    ind = num_nonzero(fact)
##  	    if len(ind):
##  		fac = num_take( triangs, ind )
##  		facCenter = num_take( verts, fac )
##  		facCenter = num_sum(facCenter, 1)/3.0
##  		l = len(facCenter)
##  		facCenter = num_sum(facCenter)/ l
##  		facesCenters.append( num_reshape(facCenter, (3,)) )
##  		if self.sesfColors:
...
##  	pos = num_array(facesCenters, num_int32)
...
##  	    if analytical == 0 :
##  		pos = num_take( self.sesv[:,:3], list(vertices) )
##  	    else:
##  		fact = num_equal( self.sesf[:,4], vertices[0] )
##  		for fi in vertices[1:]:
##  		    fact = fact + num_equal( self.sesf[:,4], fi )
##  		ind = num_nonzero(fact)
##  		fac = num_take( self.sesf[:,:3], ind )
##  		vertsFlag = num_zeros( (len(self.sesv),) )
##  		for f in fac:
##  		    vertsFlag[f[0]] = 1;
##  		    vertsFlag[f[1]] = 1;
##  		    vertsFlag[f[2]] = 1;
##  		vind = num_nonzero(vertsFlag)
##  		pos = num_take( self.sesv[:,:3], vind )
##  		labs = map(str, vind)
