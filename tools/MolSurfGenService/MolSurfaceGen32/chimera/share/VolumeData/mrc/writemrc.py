...
def closest_mrc2000_type(type):
    ...
    from numpy import int, int0, int8, int16, int32, character
    from numpy import uint, uint8, uint16, uint32
    if type in (float32, float64, float, int32, int, uint32, uint, uint16):
        ctype = float32
    elif type in (int16, uint8):
        ctype = int16
    elif type in (int8, int0, character):
        ctype = int8
    else:
