...
class Volume_Dialog_State:
  ...
  def state_from_dialog(self, volume_dialog):
    ...
    self.subregion_button = srp.subregion_button.variable.get()
    ...
    if d.focus_region:
      self.focus_volume = id(d.focus_region)
    else:
    ...
    self.histogram_volumes = [id(hp.data_region) for hp in hpanes]
  ...
  def restore_state(self, volume_dialog):
    ...
    srp.selectable_subregions.set(self.selectable_subregions,
                                  invoke_callbacks = 0)
    srp.subregion_button.variable.set(self.subregion_button,
                                      invoke_callbacks = 0)
...
class Volume_Manager_State:
  ...
  def use_relative_paths(self, directory):
    ...
    for ds, drlist in self.data_and_regions_state:
      p = ds.path
      if p:
...
def relative_path(path, dir):
  ...
  if isinstance(path, (tuple, list)):
    return tuple([relative_path(p, directory) for p in path])
    ...
    if self.version >= 2 and (path, self.grid_id) in gdcache:
      ...
      if error_message:
        ...
        if grids is None:
          grids = []
        for data in grids:
          gdcache[(path, data.grid_id)] = data # Cache using old path.
